...
Output::Output(cell2dStatic & cell) {
	// Points array
	...
	Output::initPVDArrays(cell);
}

void Output::initPVDArrays(cell2dStatic & cell) {
...
void Output::MakePVDOutput(cell2d & cell, std::string & filename) {
...
void Output::MakeCSVOutput(cell2d & cell, std::ofstream & outputGas) {
	for (int i = 0; i < max_i; i++) {
		for (int j = 0; j < max_j; j++) {
			outputGas << std::setiosflags(std::ios::fixed) << std::setprecision(10)
				...
				<< cell.at(nextN).at(i).at(j).e - (
						pow(cell.at(nextN).at(i).at(j).Vx,2)+
						pow(cell.at(nextN).at(i).at(j).Vr,2))/2 << std::endl;
		}
	}
}

void Output::MakeDynOutput(std::ofstream & outputDyn, double t, int i_sn, double x_sn, double U_sn, double boltP, double projP) {
